...
export class DiffuseMaskNode extends GenericImageNode {
    constructor(parent, x, y) {
        super(parent, x, y);
        this.central_element.textContent = "DIFFUSE";
    ...
    process() {
        if (this.inputtype) {
            ...
            ctx.filter = "none";
            ctx.drawImage(GPUPixelCeiling(this.parent.gpu, this.canvas), 0, 0);
            var out = this.canvas;
...
function GPUPixelCeiling(gpu, source) {
    const w = source.width;
    const h = source.height;

    const ceiling = gpu.createKernel(function(image) {
